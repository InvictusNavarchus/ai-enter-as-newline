...
const KeyboardHandler = {
    ...
    handleKeyDown(e) {
        if (!this.config?.enabled || Utils.isChineseInputMode(e))
            return;
    ...
    handleKeyPress(e) {
        if (!this.config?.enabled || Utils.isChineseInputMode(e))
            return;
        if (SiteHandlers.chatgpt.isActive?.())
            return; // ChatGPT uses keydown only
        const target = Utils.getEventTarget(e);
